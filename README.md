...
- 系统自动分析数据特征，给出处理建议（如反结转、末级科目筛选等）
- 测试报告可导出 Excel
...
- **安全沙箱**：用户代码在隔离环境中执行
- **API Key 安全**：AI 服务密钥已加密存储（Fernet 加密 + 独立 secret_key 文件 / 打包注入），不暴露明文，无需用户配置
- **代码验证**：AI 生成的代码经过安全检查（Dify Review Workflow 四维复核）
...
**注意**：本工具为本地部署，AI 功能通过 Dify Workflow 提供服务，无需额外配置 API Key，打开即可使用。